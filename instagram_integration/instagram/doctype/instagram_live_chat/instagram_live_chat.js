...
frappe.ui.form.on('Instagram Live Chat', {
...
function bind_realtime(frm) {
    ...
    frappe.realtime.on(`instagram_chat_${frm.doc.chat_id}`, (data) => {
...
function get_messages(frm) {
	...
	frappe.call({
		method: `instagram_integration.instagram.doctype.instagram_live_chat.instagram_live_chat.get_messages?chat_id=${frm.doc.chat_id}`,
		freeze: true,
...
function send_message(frm, messageInput) {
	...
	if(frm.doc.chat_id) {
		frappe.call({
			method: `instagram_integration.instagram.doctype.instagram_live_chat.instagram_live_chat.send_live_message`,
			args: {
...
function endLiveSession(frm) {
	frappe.call({
		method: "instagram_integration.instagram.doctype.instagram_live_chat.instagram_live_chat.end_live_session",
		args: {
			chat_id: frm.doc.chat_id
		},
		callback: function(r) {
